...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self=[super initWithFrame:frame]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(animaCenterItem:) name:@"anima" object:nil];
    }
    return self;
}
- (void)animaCenterItem:(NSNotification *)sender{
    NSDictionary *dic = sender.userInfo;
    if([[dic objectForKey:@"type"] isEqualToString:@"one"]){
        if([self.plusBtn.anima isEqualToString:@"1"]){}else{
            self.plusBtn.anima=@"1";
            ...
            [self.plusBtn.layer addAnimation:rotationAnimation forKey:@"rotationAnimation"];
        }
    }else if ([[dic objectForKey:@"type"] isEqualToString:@"two"]){
...
- (void)layoutSubviews
{
    ...
    self.plusLabel.centerY = CGRectGetMaxY(self.plusBtn.frame) + LBMagin ;
    int btnIndex = 0;
...
- (void)plusBtnDidClick
{
    ...
    if ([self.delegate respondsToSelector:@selector(tabBarPlusBtnClick:andItem:)]) {
        [self.myDelegate tabBarPlusBtnClick:self andItem:self.plusBtn];
    }
